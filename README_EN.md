...
`sockswg-bluegreen` runs two equal A/B tunnels on the same VPS, with the pure-IPv4 B tunnel isolated in a network namespace. Installation auto-detects and stores the expected location from the initial A path. An iptables connection-tracking selector moves only new SOCKS connections; established TCP connections keep using their original tunnel and drain naturally. Both paths are checked every 20 seconds for Cloudflare location, `warp=on/plus`, Google HTTP status, and a cached Gemini generation result. Two consecutive failures of the active slot trigger a switch only when the other slot qualifies. The newly active slot remains in use for as long as it stays healthy; a repaired former slot stays idle with no automatic failback.

An unhealthy inactive slot is repaired automatically. When it is healthy but not `optimal`, at most one IP-selection round starts per hour after its established connections have drained. The manager first cycles Endpoints and reconnects the existing peer. Qualification checks location, rejects Google `429`, requires Gemini to generate a real response, and prefers an egress different from the active slot. Only after repeated reroll failures does it unregister and create a candidate peer; rejected candidates are unregistered and the previous configuration is restored. Gemini results are cached for one hour per slot and public IP, while a new IP is tested immediately. `BardErrorInfo 1060`, `429`, Google block pages, timeouts, and malformed responses are unusable. If both slots fail, the manager keeps the current path instead of flapping and repairs only a drained inactive slot. Candidates also record the Google/YouTube region: matching the host's WARP location is `optimal`, while a reachable but different Google region (including `.com.hk`) is `usable` only after passing Gemini generation. Equal A/B public IPs alone do not trigger a selection round. Cloudflare still assigns the public egress, so blue/green improves availability but cannot guarantee a permanently fixed or unlimited supply of different WARP IPs.
...
## Update Information
2026.08.30 sockswg-bluegreen Gemini availability fix: health qualification now performs a low-frequency Gemini generation request instead of trusting the Google homepage alone. `BardErrorInfo 1060`, `429`, block pages, timeouts, and malformed replies are rejected. Results are cached for one hour per slot and public IP, while every new IP is checked immediately. New connections switch only when the other slot qualifies; if both fail, the current path is preserved and only the drained inactive slot is repaired.
